...
export default function Navbar({ setSearchTerm, setFilteredCategories }) {
  const [input, setInput] = useState('');
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch('https://y-balash.vercel.app/api/categories/all');
  ...
  const handleKeyDown = (event) => {
    if (event.key === 'Enter') {
      ...
      // Filter categories when Enter is pressed
      if (input.trim() === '') {
        setFilteredCategories(allCategories);
      } else {
        const filtered = allCategories.filter(category => 
      ...
      <div className="hidden md:block absolute left-1/2 transform -translate-x-1/2">
        <div className="relative w-[300px]">
          ...
          <input
            ...
            onChange={handleInputChange}
            onKeyDown={handleKeyDown}
            className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-md outline-none shadow-[0_2px_4px(rgba(0,0,0,0.05))] focus:ring-2 focus:ring-blue-300 bg-white text-gray-600 placeholder-gray-400"
          />
        </div>
      </div>
