...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const [stockRes, catRes, activityRes, ordersRes] = await Promise.all([
          axios.get(`https://y-balash.vercel.app/api/admin/low-stock-items?threshold=${threshold}`),
          axios.get("https://y-balash.vercel.app/api/admin/top-categories"),
          axios.get("https://y-balash.vercel.app/api/admin/recent-activities"),
          axios.get("https://y-balash.vercel.app/api/admin/orders-summary"),
        ]);
        ...
        setTopCategories(catRes.data?.topCategories || []);
        setRecentActivities(activityRes.data?.activities || []);
        setOrdersSummary(ordersRes.data?.data || {});
      } catch (err) {
        console.error("Error fetching data:", err);
        setError("Failed to fetch data. Please try again.");
        setLowStockItems([]);
    ...
    <div className="p-6 bg-gray-100 min-h-screen space-y-6">
      <div className="flex flex-col md:flex-row gap-6">
        <div className="flex-1 space-y-6">
          <div className="bg-white p-6 rounded shadow">
            <h2 className="text-xl font-semibold mb-4">Orders Overview</h2>
            <p><strong>Orders Today:</strong> {ordersSummary.today}</p>
            <p><strong>Orders This Week:</strong> {ordersSummary.thisWeek}</p>
            <p><strong>Orders This Month:</strong> {ordersSummary.thisMonth}</p>
          </div>
